...
void SaveLastDrawingWidget::fileButtonClicked(){
		...
		if(!src->text().isEmpty() && src->text() != ""){
			if(auto mObs = dataSaveModel.lock()){
				if(mObs->saveLastDrawingToDataFile(src->text())){
					emit showMessage_W( 
						"Az utolsó ábra ábra sikeresen mentve a " + src->text() + " fileba."
						);
					return;
				}
			}
	...
	emit showMessage_W("Az utolsó ábra mentése sikertelen");
}
void SaveLastDrawingWidget::accesAnotherDataFile(){
	QString filePath = QFileDialog::getSaveFileName(this, "tanítóminta-file választása");
	if(filePath == "" || filePath.isEmpty()){
		emit showMessage_W("A file megnyitás sikertelen, mert üres a filenév!");
		return;
	}
...
void SaveLastDrawingWidget::removeDataFileButton(){
	...
	emit showMessage_W("A gomb törlése sikertelen");
}
