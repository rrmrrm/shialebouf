...
TransitionResult CanvasManipulation::transRes = TransitionResult(
													NOTHING,
													Target(0, 0, std::shared_ptr<CanvasElement>()),
													Target(0, 0, std::shared_ptr<CanvasElement>())
												);
...
void CanvasManipulation::resetState(){
	qDebug() << "CanvasManipulation::resetState: state is INITIAL";
	state = INITIAL;
	targets.resize(0);
	Target noTarget =  Target(-1,-1, {nullptr});
	lastpressTarget =noTarget;
	transRes = {NOTHING, noTarget, noTarget};
}
...
void CanvasManipulation::applyTransition(
	...
	case ELEMENTDRAGGING:{
		...
		case QMouseEvent::MouseButtonRelease:{
			...
			state = INITIAL;
			qDebug() << "INITIAL";
			qDebug() << "and resulttype is STOPDRAGGING";
			transRes.resultType = STOPDRAGGING;
			break;
		};
		default:{
			//assert(false);
      qDebug(" CanvasManipulation::applyTransition: state unhandled in switch");
			break;
		}
		}
		break;
