...
void LineComponent::mergeLinesWhenNeeded(){
  ...
  // az első vagy utolsó vonalszegmens már lehet, más irányú mint az eddigi,
  //  ezért frissítem a rajtuk levő connectorok irányát
  updateConnectorDirs();
	//arrangeConns();
	emit linesChanged_CEW();
}
void LineComponent::dragUnselectedOverrideThis(int canvas_x, int canvas_y){
	...
	if(!isDragging){
		...
		//0 és 1 közötti valós szám. ha vonal elejre kattintotak akkor 0, ha végére akkor 1 lesz:
		...
		if(lineSegments.end() != draggingLineIt){
			if(linedivisionRatio <= 0.5){
				...
				if(!nextExists){//ha ez az utolsó szegmens:
...
std::istream& UMLClassComponent::load(std::istream& is){
	CanvasElement::load(is);
	//ellenőrzöm, hogy tényleg umlclasscomponent adatai olvasom-e:
	const std::string umlToken = "UMLClassComponent";
	std::string checkStr = "";
