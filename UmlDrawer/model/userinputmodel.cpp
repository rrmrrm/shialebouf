...
bool GenericFileUIM::readDataFromFile(){
	...
	if(!ifs.is_open()){
		std::cerr << ("WARNING: GenericFileUIM::readDataFromFile(): couldn't open file '")
					<< GetUIMIdentifier().toStdString()
					<< "' " << std::endl;
	...
	while(ifs >> checkString){
		if(ifs.fail()){
			std::cerr << "ERROR: GenericFileUIM::readDataFromFile(): couldn't read a data identifier tag" << std::endl;
			return false;
		}
		std::string nothing;
		ifs >> nothing;
		if(ifs.fail()){
			std::cerr << "ERROR: GenericFileUIM::readDataFromFile(): couldn't read "
									<< "\" : \""
									<< " string from file" << std::endl;
			return false;
		}
		...
		newVal->read(ifs);
		if(ifs.fail()){
			std::cerr << "ERROR: GenericFileUIM::readDataFromFile(): couldn't read variable" << std::endl;
			return false;
		}
